...
const SAGE = (() => {
  ...
  const SPORTS_AVAILABLE_CODES = ['NFL', 'NBA', 'MLB', 'NHL', 'MMA'];
  const SPORTS_DEFAULT_PREFERENCES = {
    ...
    allowedSports: [...SPORTS_AVAILABLE_CODES],
  };
  ...
  function normalizeSportCode(value) {
    ...
    if (SPORTS_AVAILABLE_CODES.includes(raw)) return raw;
  ...
  function normalizeSportsPreferences(input = {}) {
    ...
    const allowedSports = Array.isArray(input.allowedSports) && input.allowedSports.length
      ? [...new Set(input.allowedSports.map(normalizeSportCode).filter(code => SPORTS_AVAILABLE_CODES.includes(code)))]
      : [...SPORTS_DEFAULT_PREFERENCES.allowedSports];
    return {
  ...
  function readSportsPreferencesFromUI() {
    ...
    const allowedSports = [...document.querySelectorAll('[data-sports-code]')]
      .filter(el => el.checked)
      .map(el => el.getAttribute('data-sports-code'));
  ...
  function loadSportsPreferences() {
    ...
    const allowedSports = new Set((prefs.allowedSports || SPORTS_DEFAULT_PREFERENCES.allowedSports).map(normalizeSportCode));
    document.querySelectorAll('[data-sports-code]').forEach(el => {
      el.checked = allowedSports.has(normalizeSportCode(el.getAttribute('data-sports-code')));
    });
    return prefs;
  }
