...
app.post('/api/sheets/mark-outcome', async (req, res) => {
  try {
    const { sessionId, identifier, outcome, pnl, runningRoi, agentWeight, token, sheetId } = req.body || {};
    if (token && sheetId) {
      await ensureSheetsDirect(token, sheetId).catch(() => {});
      const range = "'Sports Picks'!A:S";
      const resp = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${encodeURIComponent(range)}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!resp.ok) throw new Error(`Sheets read ${resp.status}`);
      const data = await resp.json();
      const rows = data.values || [];
      if (rows.length < 2) return res.json({ success: false, reason: 'no rows' });
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const matchSession = !sessionId || rowSession === String(sessionId);
        const matchIdentifier = !target || rowKey.includes(target) || target.includes(rowKey) || rowPick.includes(target);
        if (matchSession && matchIdentifier) { matched = i; break; }
      }
      if (matched < 0) return res.json({ success: false, reason: 'match not found' });
      const row = rows[matched].slice();
      if (outcomeCol >= 0) row[outcomeCol] = outcome === 'win' ? 'WIN ✅' : 'LOSS ❌';
      if (pnlCol >= 0 && pnl !== undefined) row[pnlCol] = pnl;
      if (roiCol >= 0 && runningRoi !== undefined) row[roiCol] = runningRoi;
      if (weightCol >= 0 && agentWeight !== undefined) row[weightCol] = agentWeight;
      await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${encodeURIComponent(`'Sports Picks'!A${matched + 1}:S${matched + 1}`)}?valueInputOption=USER_ENTERED`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    ...
    if (!sheets) return res.status(401).json({ error: 'Not authorized' });
    await ensureSheets(sheets);
    const existing = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: "'Sports Picks'!A:S" });
    const rows = existing.data.values || [];
    if (rows.length < 2) return res.json({ ok: false, reason: 'no rows' });
    const headers = rows[0].map(h => String(h || ''));
    const sessionCol = headers.findIndex(h => h.toLowerCase().includes('session'));
    const gameCol = headers.findIndex(h => h.toLowerCase() === 'game');
    const pickCol = headers.findIndex(h => h.toLowerCase() === 'pick');
    ...
    if (matched < 0) return res.json({ ok: false, reason: 'match not found' });
    const row = rows[matched].slice();
    if (outcomeCol >= 0) row[outcomeCol] = outcome === 'win' ? 'WIN ✅' : 'LOSS ❌';
    if (pnlCol >= 0 && pnl !== undefined) row[pnlCol] = pnl;
    if (roiCol >= 0 && runningRoi !== undefined) row[roiCol] = runningRoi;
    if (weightCol >= 0 && agentWeight !== undefined) row[weightCol] = agentWeight;
    await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: `'Sports Picks'!A${matched + 1}:S${matched + 1}`, valueInputOption: 'USER_ENTERED', resource: { values: [row] } });
    res.json({ success: true, mode: 'server' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
